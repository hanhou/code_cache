...
for imec in imec_folders:
    ...
    run, g, prb = m.group(1), m.group(2), m.group(3)
    ...
    # Do catGT
    cmd = f'{catgt_exe} -dir={data_root_path} -prb_fld -run={run} -g={g} -prb={prb} -t=0,{t_max} {catgt_cmd} {output_cmd}'
    err = subprocess.call(cmd.split())  # Not using split(' ') to take care of more than one spaces between strings

    if not err:
        n_probe += 1
        print(f'{n_probe} done: {imec}, t_max = {t_max}')
    else:
