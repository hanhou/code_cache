function f = plotLFPpower(F, allPowerEst, dispRange, marginalChans, freqBands, lfpSurface, antiStaggering)
...
imagesc(F(dispF), depth, 10*log10(allPowerEst(:,dispF)));
xlim(dispRange);
xlabel('frequency (Hz)');
...
ylabel('power (dB)');
set(ax, 'XTick', []);
hleg = legend(arrayfun(@num2str, marginalChans*10*(1+antiStaggering), 'un', 0));
